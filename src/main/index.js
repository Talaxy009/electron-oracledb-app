...
ipcMain.on("getTeachers", (event) => {
  DBC.execute("select tname from teacher", (err, result) => {
    if (err) {
      console.error(err.message);
      return;
    }
...
ipcMain.on("createUser", (_event, userId) => {
  DBC.execute(`CREATE user S${userId} IDENTIFIED BY pwd${userId}`, (err) => {
    if (err) {
      console.error(err.message);
      return;
    }
...
ipcMain.on("openStudentPage", (event, user, password) => {
  ...
  DataBase.getConnection(config, (err, connection) => {
    if (err) {
      console.error(err.message);
